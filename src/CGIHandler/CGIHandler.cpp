...
/*   Updated: 2024/04/11 15:05:28 by rmarceau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
CGIHandler::CGIHandler(HTTPRequest *request, t_server &server, std::string path) : _request(request), _server(server), _path(path) {
    (void)_request;
    return;
}
...
std::string CGIHandler::executeCGI(void) {
    ...
    if (pid == -1) {
        ...
        const char* interpreter = "/usr/bin/python3"; // Or "/usr/bin/env python3"
